#!/bin/bash
# =============================================================================
# AI Studio Thailand — Content Creator Local Setup
# ติดตั้งระบบ AI สำหรับครีเอเตอร์
# =============================================================================
...
echo -e "${BOLD}${CYAN}"
echo "  ╔══════════════════════════════════════════════════════════╗"
echo "  ║     AI Studio Thailand — Content Creator Setup          ║"
echo "  ║     ระบบ AI สำหรับครีเอเตอร์                                ║"
echo "  ╚══════════════════════════════════════════════════════════╝"
echo -e "${NC}"
...
sed -i.bak "s|DIFY_SECRET_KEY=change-me-to-random-string|DIFY_SECRET_KEY=$DIFY_SECRET|" "$SCRIPT_DIR/.env"
...
read -rp "  Domain (e.g. ai.mychannel.com) [localhost]: " INPUT_DOMAIN
...
read -rp "  TikTok Access Token (optional): " TIKTOK_TOKEN
if [ -n "$TIKTOK_TOKEN" ]; then
    sed -i.bak "s|TIKTOK_ACCESS_TOKEN=your-tiktok-token|TIKTOK_ACCESS_TOKEN=$TIKTOK_TOKEN|" "$SCRIPT_DIR/.env"
    rm -f "$SCRIPT_DIR/.env.bak"
fi

read -rp "  Instagram Access Token (optional): " IG_TOKEN
if [ -n "$IG_TOKEN" ]; then
    sed -i.bak "s|INSTAGRAM_ACCESS_TOKEN=your-instagram-token|INSTAGRAM_ACCESS_TOKEN=$IG_TOKEN|" "$SCRIPT_DIR/.env"
    rm -f "$SCRIPT_DIR/.env.bak"
fi

read -rp "  Facebook Page Token (optional): " FB_TOKEN
if [ -n "$FB_TOKEN" ]; then
    sed -i.bak "s|FACEBOOK_PAGE_TOKEN=your-facebook-page-token|FACEBOOK_PAGE_TOKEN=$FB_TOKEN|" "$SCRIPT_DIR/.env"
    rm -f "$SCRIPT_DIR/.env.bak"
fi
...
for i in $(seq 1 30); do
    if curl -s -o /dev/null http://localhost:7778 2>/dev/null; then
        success "Dify is ready / Dify พร้อมแล้ว"; break
    fi
    echo -ne "\r  Waiting for Dify... ($i/30)"; sleep 2
done
echo ""
...
if [ -f "$PARENT_DIR/import-workflows.sh" ]; then
    bash "$PARENT_DIR/import-workflows.sh" http://localhost:7777 creator || warn "Auto-import skipped. Import manually after n8n setup."
else
...
echo -e "  ${CYAN}Dify (AI):${NC}         http://localhost:7778"
echo ""
echo -e "  ${YELLOW}Workflows included / Workflows ที่มี:${NC}"
echo "    - Content Generator (สร้างคอนเทนต์อัตโนมัติ)"
echo "    - Content Calendar (ปฏิทินคอนเทนต์)"
echo "    - Engagement Report (รายงาน engagement)"
echo ""
