#!/bin/bash
# =============================================================================
# AI Studio Thailand — Healthcare / Clinic Local Setup
# ติดตั้งระบบ AI สำหรับคลินิก / โรงพยาบาล
# =============================================================================
...
echo -e "${BOLD}${CYAN}"
echo "  ╔══════════════════════════════════════════════════════════╗"
echo "  ║     AI Studio Thailand — Healthcare Setup               ║"
echo "  ║     ระบบ AI สำหรับคลินิก / โรงพยาบาล                       ║"
echo "  ╚══════════════════════════════════════════════════════════╝"
echo -e "${NC}"
...
read -rp "  Domain (e.g. ai.myclinic.com) [localhost]: " INPUT_DOMAIN
...
read -rp "  Admin LINE User ID: " LINE_ADMIN
if [ -n "$LINE_ADMIN" ]; then
    sed -i.bak "s|ADMIN_LINE_USER_ID=your-line-user-id|ADMIN_LINE_USER_ID=$LINE_ADMIN|" "$SCRIPT_DIR/.env"
    rm -f "$SCRIPT_DIR/.env.bak"
fi
...
if [ -f "$PARENT_DIR/import-workflows.sh" ]; then
    bash "$PARENT_DIR/import-workflows.sh" http://localhost:7777 healthcare || warn "Auto-import skipped. Import manually after n8n setup."
else
...
echo "    - Appointment Bot (บอทนัดหมาย LINE)"
echo "    - Patient Reminder (เตือนนัดผู้ป่วย)"
echo "    - Daily Clinic Report (รายงานคลินิกรายวัน)"
echo ""
echo -e "  ${YELLOW}Flowise Flows / Flowise ที่มี:${NC}"
echo "    - Drug Info RAG (ค้นหาข้อมูลยา)"
echo ""
