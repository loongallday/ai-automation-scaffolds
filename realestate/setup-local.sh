#!/bin/bash
# =============================================================================
# AI Studio Thailand — Real Estate Local Setup
# ติดตั้งระบบ AI สำหรับอสังหาริมทรัพย์
# =============================================================================
...
echo -e "${BOLD}${CYAN}"
echo "  ╔══════════════════════════════════════════════════════════╗"
echo "  ║     AI Studio Thailand — Real Estate Setup              ║"
echo "  ║     ระบบ AI สำหรับอสังหาริมทรัพย์                           ║"
echo "  ╚══════════════════════════════════════════════════════════╝"
echo -e "${NC}"
...
read -rp "  Domain (e.g. ai.myproperty.com) [localhost]: " INPUT_DOMAIN
...
if [ -f "$PARENT_DIR/import-workflows.sh" ]; then
    bash "$PARENT_DIR/import-workflows.sh" http://localhost:7777 realestate || warn "Auto-import skipped. Import manually after n8n setup."
else
...
echo "    - Property Inquiry Bot (บอทตอบคำถามอสังหา LINE)"
echo "    - Appointment Scheduler (นัดดูห้อง/บ้าน)"
echo "    - Lead Scoring (ให้คะแนนลูกค้า)"
echo ""
echo -e "  ${YELLOW}Flowise Flows / Flowise ที่มี:${NC}"
echo "    - Listing RAG (ค้นหาทรัพย์สิน)"
echo ""
