#!/bin/bash
# =============================================================================
# AI Studio Thailand — Restaurant Local Setup
# ติดตั้งระบบ AI สำหรับร้านอาหาร
# =============================================================================
...
echo -e "${BOLD}${CYAN}"
echo "  ╔══════════════════════════════════════════════════════════╗"
echo "  ║     AI Studio Thailand — Restaurant Setup               ║"
echo "  ║     ระบบ AI สำหรับร้านอาหาร                                ║"
echo "  ╚══════════════════════════════════════════════════════════╝"
echo -e "${NC}"
...
read -rp "  Domain (e.g. ai.myrestaurant.com) [localhost]: " INPUT_DOMAIN
...
if [ -f "$PARENT_DIR/import-workflows.sh" ]; then
    bash "$PARENT_DIR/import-workflows.sh" http://localhost:7777 restaurant || warn "Auto-import skipped. Import manually after n8n setup."
else
...
echo "    - LINE Order Bot (บอทรับออเดอร์ LINE)"
echo "    - Inventory Alert (แจ้งเตือนวัตถุดิบ)"
echo "    - Review Reply (ตอบรีวิวอัตโนมัติ)"
echo "    - Daily Sales Report (รายงานยอดขายรายวัน)"
echo ""
