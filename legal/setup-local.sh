#!/bin/bash
# =============================================================================
# AI Studio Thailand — Legal Office Local Setup
# ติดตั้งระบบ AI สำหรับสำนักงานกฎหมาย
# =============================================================================
...
echo -e "${BOLD}${CYAN}"
echo "  ╔══════════════════════════════════════════════════════════╗"
echo "  ║     AI Studio Thailand — Legal Office Setup             ║"
echo "  ║     ระบบ AI สำหรับสำนักงานกฎหมาย                          ║"
echo "  ╚══════════════════════════════════════════════════════════╝"
echo -e "${NC}"
...
read -rp "  Domain (e.g. ai.mylawfirm.com) [localhost]: " INPUT_DOMAIN
...
for i in $(seq 1 30); do
    if curl -s -o /dev/null http://localhost:7778 2>/dev/null; then
        success "Flowise is ready / Flowise พร้อมแล้ว"; break
    fi
    echo -ne "\r  Waiting for Flowise... ($i/30)"; sleep 2
done
echo ""

for i in $(seq 1 15); do
    if curl -s -o /dev/null http://localhost:7782 2>/dev/null; then
        success "Qdrant is ready / Qdrant พร้อมแล้ว"; break
    fi
    echo -ne "\r  Waiting for Qdrant... ($i/15)"; sleep 2
done
echo ""
...
if [ -f "$PARENT_DIR/import-workflows.sh" ]; then
    bash "$PARENT_DIR/import-workflows.sh" http://localhost:7777 legal || warn "Auto-import skipped. Import manually after n8n setup."
else
...
echo -e "  ${CYAN}Qdrant (Vectors):${NC}  http://localhost:7782/dashboard"
echo ""
echo -e "  ${YELLOW}Workflows included / Workflows ที่มี:${NC}"
echo "    - Document Draft (ร่างเอกสาร)"
echo "    - PDPA Compliance Check (ตรวจสอบ PDPA)"
echo ""
echo -e "  ${YELLOW}Flowise Flows / Flowise ที่มี:${NC}"
echo "    - Contract Analysis (วิเคราะห์สัญญา)"
echo "    - Legal Search RAG (ค้นหากฎหมาย)"
echo ""
